<?php

$string['batch'] = 'Tasques per lots';
$string['errorlog'] = 'Registre d\'errors';
$string['errorlog_filter'] = 'Filtre';
$string['errorlog_notfound'] = 'No s\'ha trobat el fitxer de registre d\'errors.';
$string['errorlog_path'] = 'Camí del fitxer de registre d\'errors';
$string['errorlog_refresh'] = 'Refresca';
$string['local'] = 'Local';
$string['local:enrolled'] = 'Inscrit';
$string['othercourses'] = 'Altres cursos...';
$string['profiling'] = 'Profiling';
$string['profiling_course'] = 'Curs';
$string['profiling_day'] = 'Dia';
$string['profiling_enable'] = 'Habilita profiling';
$string['profiling_hits'] = 'Peticions';
$string['profiling_hour'] = 'Hora';
$string['profiling_month'] = 'Mes';
$string['profiling_script'] = 'Script';
$string['profiling_time'] = 'Temps';
$string['profiling_year'] = 'Any';
$string['settings'] = 'Paràmetres';
